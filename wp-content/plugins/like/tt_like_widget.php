...
//customised for XTRAIN project
    ob_start();
    if(function_exists('wp_email')) { email_link(); }
    $captured_markup = ob_get_contents();
    ob_end_clean();
    $button .= $captured_markup;
    //end customised
	...
	$button .= '<iframe src="http://www.facebook.com/plugins/like.php?href='.$url.'" scrolling="no" frameborder="0" allowTransparency="true" style="margin-bottom: -7px !important; border:none; overflow:hidden; width:'.$tt_like_settings['width'].'px; height: '.$tt_like_settings['height'].'px; align: '.$align.'; margin: '.$margin.'"></iframe>';
    }
